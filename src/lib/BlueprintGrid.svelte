...
{#if !isMobile}
	<div class="pointer-events-none fixed inset-0 z-0">
		{#if contentRect}
			...
			<div
				class="pointer-events-none absolute top-0 w-px bg-neutral-200"
		...
		{#if panelRect}
			...
			{#if hoveredEdge === 'panel-left'}
				<div
					class="absolute top-12 flex -translate-x-1/2 items-center gap-1 rounded-sm bg-white px-1.5 py-0.5 font-mono text-[10px] text-purple-500 shadow-sm"
		...
		<!-- diagonal hatch gutters — fill the gap between the boxed
		     ...
		     real content/panel edge -->
		{#if contentRect && contentRect.left > CHROME_WIDTH}
			<div
				class="absolute top-0 bg-[repeating-linear-gradient(315deg,var(--pattern-fg)_0,var(--pattern-fg)_1px,transparent_0,transparent_50%)] bg-size-[10px_10px] bg-fixed [--pattern-fg:var(--color-neutral-900)]/5"
