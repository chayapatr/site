...
<div class="flex flex-wrap items-center gap-1 px-1 pt-2 pb-1">
	...
	>
		<div class="relative h-3.5 w-6 rounded-full transition-colors {listView ? 'bg-neutral-800' : 'bg-neutral-200'}">
			<div class="absolute top-0.5 h-2.5 w-2.5 rounded-full bg-white transition-all {listView ? 'left-3' : 'left-0.5'}"></div>
		</div>
		List
	</button>
		...
		{#each visibleGroups as group (group.year)}
			...
			{#each group.projects as project (project.title)}
				...
				>
					...
					<span class="ml-1 min-w-0 flex-1 truncate text-[13px] text-neutral-700">{project.title}</span>
