...
<div
	class="glass-dark absolute flex flex-col overflow-hidden rounded-md border text-xs shadow-sm"
	class:border-neutral-600={win.focused}
	class:border-neutral-800={!win.focused}
	style="left:{localX}px; top:{localY}px; width:{localW}px; height:{localH}px; z-index:{win.zIndex};"
	onmousedown={onFocus}
	role="none"
>
	<!-- Title bar — mirrors Block.svelte's titlebar exactly -->
	<div
	...
	>
		<div class="no-drag flex gap-2">
			<button class="text-neutral-500 hover:text-red-400" aria-label="Close" onclick={onClose}
				>[x]</button
			>
			<button
				class="text-neutral-500 hover:text-yellow-400"
				aria-label="Minimize"
				onclick={onMinimize}>[-]</button
			>
			<button
				class="text-neutral-500 hover:text-green-400"
